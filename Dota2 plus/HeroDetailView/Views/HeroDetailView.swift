            ...
            VStack(spacing: 0) {
                ...
                HStack {
                    Text("For more information go to")
                        .font(.system(size: 12))
                    Link("\(hero.info.localizedName)", destination: Constants.Urls.urlHeroLink(hero.info.localizedName))
                        .bold()
                        .foregroundColor(.blue)
                }.padding(.vertical, 40)
            }
...
struct HeroDetailsViewControlle_Previews: PreviewProvider {
    static var previews: some View {
        HeroDetailView(viewModel: HeroDetailViewModel(apiService: HeroDetailService(dotaService: DotaApiService(urlSession: .shared)), hero: HeroOrganizationModel(info: HeroData(id: 1,
